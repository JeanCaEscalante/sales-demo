                            ...
                            ->afterStateUpdated(function ($state, Get $get, Set $set) {
                               // $user = User::find(Auth::user()->id);
                                ...
                                if ($state === 'bill') {
                                    ...
                                    $series = 'FTA-'.str_replace('-', '', $date);
                                    $set('series', $series);

                                    $todayInvoices = Sale::Authenticated()->where('series', $series)->get();
                                    // buscar todas las facturas generadas por este cliente el dia de hoy

                                    $maxNumber = 0;
                                    foreach ($todayInvoices as $invoices) {
                                        $currentNumber = (int) str_replace($series.'-', '', $invoices->invoice_number ?? '0');
                                    ...
                                    $set('invoice_number', $series.'-'.$invoice_number); 
                                    
                                } else {
                                    $series = 'TKT-'.str_replace('-', '', $date);
                                    $set('series', $series);

                                    // buscar todas las facturas generadas por este cliente el dia de hoy
                                    $todayInvoices = Document::where('user_id', Auth::user()->id)->where('invoice_series_code', $get('invoice_series_code'))->whereIn('type_status', [TypeStatus::CORRECT, TypeStatus::ACCEPTED_ERRORS, TypeStatus::INCORRECT])->get();
                                    $maxNumber = 0;
                                    foreach ($todayInvoices as $invoices) {
                            ...
                            ->schema([
                                Forms\Components\Tabs::make('service_primary')
                                    ->tabs([
                                        ...
                                        Forms\Components\Tabs\Tab::make('Detalle Articulo')
                                            ->schema([
